...
class LayeredHypergraphManager:
    def draw_layered_hypergraph(self, hyperedges):
        ...
        for edge_data in hyperedges.values():
            all_layers.add(
                edge_data.get("layer", 0)
            )  # Default layer set to 0 (lowest)
        ...
        layer_colors = [
            custom_colors[i % len(custom_colors)] for i in range(num_layers)
        ]
        ...
        if num_layers == 1:
            ...
            layer_z_mapping = {
                layer: (num_layers - 1 - idx) * (max_z / (num_layers - 1))
        ...
        for layer, edges in layer_groups.items():
            ...
            for edge in edges:
                ...
                if len(nodes) == 1:
                    ...
                    min_dist = (
                        0.5  # Minimum distance between single-node hyperedges
                    )
                    for other_edge, other_pos in edge_positions.items():
                        dist = np.linalg.norm(
                            np.array(base_pos) - np.array(other_pos)
                        )
                        if dist < min_dist:
                            # Offset the position
                            offset = (min_dist - dist) * np.array(
                                [1, 0]
                            )  # Offset along x-axis
                            base_pos = (
                                base_pos[0] + offset[0],
                    ...
                    edge_pos = np.mean(
                        [pos_2d[node] for node in nodes], axis=0
                    )
        ...
        for layer in sorted_layers:
            z = layer_z_mapping[layer]
            vertices = [(-3, -3, z), (3, -3, z), (3, 3, z), (-3, 3, z)]
            ax.add_collection3d(
                Poly3DCollection(
                    [vertices],
                    alpha=0.4,
                    color=layer_colors[sorted_layers.index(layer)],
                )
            )
        ...
        for edge_name, edge_data in hyperedges.items():
            ...
            points = np.array(
                [node_positions[(node, layer)][:2] for node in node_list]
            )  # 2D points (x, y)

            # Define size parameters
            size_multiplier = 1.0  # Consistent multiplier across all layers
            min_size = (
                1.0  # Increased minimum size to match default ellipse size
            )
            min_max_dist = (
                0.3  # Minimum max_dist to ensure reasonable ellipse size
            )
        ...
        for (node, layer), (x, y, z) in node_positions.items():
            ...
            ax.text(
                ...
                va="bottom",
                bbox=dict(
                    facecolor="none", alpha=0.7, edgecolor="none", pad=1
                ),
                zorder=1000,
            )
        ...
        for layer_idx in range(num_layers):
            legend_patches.append(
                plt.Line2D(
                    ...
                    markerfacecolor=layer_colors[
                        layer_idx % len(layer_colors)
