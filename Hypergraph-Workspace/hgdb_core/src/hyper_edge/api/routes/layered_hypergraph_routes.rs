...
pub fn configure_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/api")
            ...
            .service(
                web::resource("/layered-hypergraphs/{id}/visualize")
                    .route(web::post().to(layered_hypergraph_handler::visualize_layered_hypergraph))
            )
            .route("/health", web::get().to(layered_hypergraph_handler::health_check))
    );
}